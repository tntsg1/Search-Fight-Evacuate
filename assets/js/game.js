...
let starfield = [];
const shimmerRings = [
  {x:0.2,y:0.34,r:140,color:'#12d4ff',glow:26,offset:0.0},
  {x:0.78,y:0.28,r:120,color:'#ff52ff',glow:24,offset:1.2},
  {x:0.64,y:0.74,r:160,color:'#ffd166',glow:22,offset:2.4},
];
...
function drawBG(dt=16, now=performance.now()){
  ...
  shimmerRings.forEach(r=>{
    const pulse= Math.sin(time*1.2 + r.offset)*8;
    neonCircle(r.x*W, r.y*H, r.r + pulse, r.color, 0.08, r.glow);
  });
  ctx.restore();
...
/* ======= FX：刀光/火花/屏幕震动 ======= */
let slashFX=[];   // {x,y,angle,t,dur,r,arc}
...
function meleeWeapon(){ return {id:'melee',name:'MELEE', melee:true, dmg:70, delay:380, range:46, arc:1.35}; }
...
function spawnSparks(x,y,color='#9ecbff',n=10){
  for(let i=0;i<n;i++){
...
function meleeAttack(visualOnly=false){
  // 加入刀光与冲击波
  slashFX.push({x:player.x,y:player.y,angle:player.angle,t:0,dur:220,r:weapon.range+16,arc:weapon.arc});
  addShake(10,140);
  ...
  for(const e of enemies){
    ...
    if(d <= weapon.range + e.r){
      ...
      if(da <= weapon.arc*0.5){
        ...
        e.x += nx*8; e.y += ny*8;
        e.hitFlash = 140;
        spawnSparks(e.x+nx*e.r*0.6, e.y+ny*e.r*0.6, '#ff84ff', 12);
        hits++;
...
function renderSlashAndSparks(glow=20){
  ...
  for(const s of slashFX){
    ...
    // 背板：淡色扇形
    ctx.save();
    ctx.globalAlpha = 0.18*(1-pct);
    ctx.fillStyle = '#00f7ff';
    ctx.shadowBlur = glow; ctx.shadowColor='#00f7ff';
    ctx.beginPath();
    ctx.moveTo(s.x, s.y);
    ...
    // 刀锋轨迹：亮色弧线
    ctx.save();
    ctx.globalAlpha = 0.9*(1-pct);
    ctx.lineWidth = 6;
    ctx.strokeStyle = '#ff1cf7';
    ctx.shadowBlur = glow*1.2; ctx.shadowColor='#ff1cf7';
    ctx.beginPath();
    ...
    if(pct<0.35){
      ctx.save();
      ctx.globalAlpha = (0.35-pct)*1.8;
      ctx.lineWidth = 3;
      ctx.strokeStyle = '#68f8a7'; ctx.shadowBlur = glow; ctx.shadowColor='#68f8a7';
      ctx.beginPath();
      ctx.arc(s.x + Math.cos(s.angle)*22, s.y + Math.sin(s.angle)*22, 10 + pct*26, 0, Math.PI*2);
      ctx.stroke();
      ctx.restore();
    }
  }
